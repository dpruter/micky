...
from proxy.views import rtv_proxy,rtv_proxy_cached,covr_proxy
...
DIRECT_SUBMIT_STATES = ['CA','NV','WA','AZ','LA','MN']

#languages that allow direct submit
DIRECT_SUBMIT_LANGUAGES = ['en']
...
def register(request):
    ...
    if 'state' in request.GET:
        ...
        if (state in DIRECT_SUBMIT_STATES) and (request.LANGUAGE_CODE in DIRECT_SUBMIT_LANGUAGES) and (not request.GET.has_key('no_redirect')):
        ...
        staterequirements = rtv_proxy_cached('GET',{'home_state_id':state,'lang':request.LANGUAGE_CODE},
            '/api/v3/state_requirements.json')
...
def register_direct(request,state_abbr):
    ...
    if not (state in DIRECT_SUBMIT_STATES and request.LANGUAGE_CODE in DIRECT_SUBMIT_LANGUAGES):
        redirect_url = reverse('registrant.views.register')
        params = request.GET.copy()
        params['state'] = state
        if params:
